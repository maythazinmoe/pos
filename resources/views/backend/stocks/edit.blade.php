...
@section('content')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
    	<div class="row">
    		<div class="col">
      		<h1 class="h3 mb-0 text-gray-800">Stock Edit Form</h1>
    		</div>
    	</div>
    </div>
      	...
      	<div class="col-md-12">
      		<form action="{{route('stocks.update',$stock->id)}}" method="post">
			     ...
			     <div class="form-group row {{ $errors->has('date') ? 'has-error' : '' }}">
			        ...
			        <div class="col-sm-5">
			          <input type="date" class="form-control" id="inputDate" name="date"  value="{{$stock->date}}">
			       ...
			       <div class="form-group row {{ $errors->has('quantity') ? 'has-error' : '' }}">
			        ...
			        <div class="col-sm-5">
			          <input type="number" class="form-control" id="inputQuantity" name="quantity" value="{{$stock->quantity}}">
			      ...
			      <div class="form-group row {{ $errors->has('buy_price') ? 'has-error' : '' }}">
			        ...
			        <div class="col-sm-5">
			          <input type="number" class="form-control" id="inputBuyPrice" name="buy_price" value="{{$stock->buy_price}}">
			          <span class="text-danger">{{ $errors->first('buy_price') }}</span>
			        </div>
			      </div>
			     {{-- 
			      <div class="form-group row {{ $errors->has('status') ? 'has-error' : '' }}">
			        <label for="inputStatus" class="col-sm-2 col-form-label">Buy Price</label>
			        <div class="col-sm-5">
			          <input type="number" class="form-control" id="inputStatus" name="status"  value="{{$stock->status}}">
